...
function invoice(__typeOfInvoice, __paymntMethod, __ob__, __vatRate) {
  ...
  facture = journal(this.__dates__, this.__codes__, this.__amount__, this.__ref__, this.__reductRate__, this.__vatRate__)
  var object = [facture[0].date, facture[0].comments]
  for (let i = 1; i < facture.length; i++) {
    object.push({
      codes: (facture[i].code),
      label: chart_of_accounts(facture[i].code),
      credit: (facture[i].credit || 0),
      debit: (facture[i].debit || 0)
    })
  }
  return object
}
